...
public class PlanExecutor {
     ...
     private void setupPlanTrigger(JsonObject plan) {
          ...
          //     logger.info("Triggering plan: " + plan.get("name").getAsString() + " at " + new
          //     SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())); logger.info("Frequency
          //     Type: " + type);
          if (value != null) {
          ...
          switch (type) {
               ...
               case "hours":
                    if (value != null) {
                         long interval = value * 60 * 60 * 1000; // Convert hours to milliseconds
                         timer.scheduleAtFixedRate(new TimerTask() {
                              @Override
                              public void run() {
                                   triggerPlan(plan);
               ...
               case "dates":
                    if (plan.has("frequency_value") && plan.get("frequency_value").isJsonArray()) {
                         JsonArray frequencyValues = plan.get("frequency_value").getAsJsonArray();
                         List<Integer> datesList = new ArrayList<>();

                         // Convert JsonArray to List<Integer>
                         for (int i = 0; i < frequencyValues.size(); i++) {
                              datesList.add(frequencyValues.get(i).getAsInt());
                         }
                         ...
                         timer.scheduleAtFixedRate(new TimerTask() {
                              ...
                              public void run() {
                                   ...
                                   // Check if today is one of the specified days (e.g., 1st or
                                   // 25th)
                                   if (datesList.contains(currentDay)) {
                                        triggerPlan(plan);
                                   }
                              }
                         }, getMidnightDelay(), 24 * 60 * 60 * 1000); // Check daily
                    } else {
                         logger.info("Cannot schedule plan with null or empty frequency value.");
                    }
                    break;

               case "one_time":
                    // Run the plan once immediately
                    triggerPlan(plan);
                    break;

               case "always_on":
                    triggerPlan(plan); // Execute the plan immediately
                    timer.scheduleAtFixedRate(new TimerTask() {
                         @Override
                         public void run() {
                              triggerPlan(plan);
                         }
                    }, 0, 5 * 60 * 1000); // Re-run every 5 minutes
                    break;

               case "seconds":
                    if (value != null) {
                         long interval = value * 1000; // Convert seconds to milliseconds
                         timer.scheduleAtFixedRate(new TimerTask() {
                              @Override
                              public void run() {
                                   triggerPlan(plan);
         ...
         logger.info(isMasterAppEnabled+"isMasterAppEnabled");
          ...
          switch (planName) {
               case "Plan A":
                    ...
                    //               type: " + frequencyType + " and value: " + frequencyValue);
                    processPlanA();
                    break;
               case "Plan B":
                    processPlanB();
                    //                // Trigger specific actions for Plan B
                    //            	logger.info("Executing actions for Plan B with frequency
                    //            type: " + frequencyType + " and value: " + frequencyValue);
                    break;
               case "Plan C":
                    processPlanC();
                    //                // Trigger specific actions for Plan C
                    //            	logger.info("Executing actions for Plan C with frequency
                    //            type: " + frequencyType + " and value: " + frequencyValue);
                    break;
               case "Plan D":
                    processPlanD();
                    //                // Trigger specific actions for Plan D
                    //            	logger.info("Executing actions for Plan D with frequency
                    //            type: " + frequencyType + " and value: " + frequencyValue);
                    break;
               case "Plan F":
                    processPlanF();
                    ...
                    for (File fileUpload : files) {
                         if (fileUpload.isFile()) {
                              ...
                              if (uploadSuccess) {
                                   ...
                                   // Delete the file from the spvDocsFolderPath after successful
                                   // upload
                                   if (file.delete()) {
